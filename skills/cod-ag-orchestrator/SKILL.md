...
You do **not** decide what happens next. `codag next` does. It reads the run
...
Announce once: "Running the cod-ag pipeline." Then work.

`cod-ag:cod-ag-conventions` holds the artifact shapes and the CLI, if you
need them.
...
```bash
python "${CLAUDE_PLUGIN_ROOT}/scripts/codag.py" init --prompt "<the user request>"
python "${CLAUDE_PLUGIN_ROOT}/scripts/codag.py" init --spec <path>
```
...
```bash
python "${CLAUDE_PLUGIN_ROOT}/scripts/codag.py" next --json
```
