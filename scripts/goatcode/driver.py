...
`goatcode next` has always decided what happens; the only thing Claude Code
    ...
    def receipt(self, receipt):
        ...
        if receipt.denials:
            self.write(
                ...
                ".goatcode/config.yaml to let agents run commands unattended.".format(
                    len(receipt.denials)
                )
            )
...
class Driver:
    ...
    def cli(self, command):
        """Run one rendered goatcode command in this process.
...
def cli_module():
    """``scripts/goatcode.py``, however this process happens to have loaded it.
    ...
    if _CLI is not None:
        return _CLI
    for name in ("__main__", "goatcode_cli"):
        module = sys.modules.get(name)
        if module is not None and hasattr(module, "main") and hasattr(module, "build_parser"):
            _CLI = module
            return _CLI
    path = pathlib.Path(__file__).resolve().parent.parent / "goatcode.py"
    spec = importlib.util.spec_from_file_location("goatcode_cli", path)
    module = importlib.util.module_from_spec(spec)
    sys.modules["goatcode_cli"] = module
