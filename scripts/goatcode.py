#!/usr/bin/env python3
"""goat-code command line - the deterministic half of the pipeline.
...
their tokens on judgement instead of rediscovering git plumbing.
    ...
    python goatcode.py init --prompt "add magic-link login"
    python goatcode.py plan validate
    python goatcode.py wave next
    python goatcode.py worktree create S1
    python goatcode.py brief S1
    python goatcode.py merge
    python goatcode.py gates run
    python goatcode.py verify-package
    python goatcode.py finish

Run ``python goatcode.py --help`` for the full list. Every subcommand accepts
...
from goatcode import (  # noqa: E402
    brief as briefmod,
    debuglog,
    diffpkg,
...
from goatcode.run import Run, RunError  # noqa: E402
...
def resolve_repo(args):
    """The main work tree, where .goatcode/ lives.
...
def cmd_run(args):
    ...
    try:
        ...
        console.write("")
        console.write("interrupted. `goatcode run --resume` picks up where this stopped.")
        return EXIT_FAIL
...
def cmd_init(args):
    ...
    if gitignored:
        lines.append("")
        lines.append(
            "added goat-code's entries to .gitignore - an uncommitted change in "
...
def cmd_verify_package(args):
    ...
    if not pathlib.Path(where).exists():
        raise CliError("no integration worktree; run 'goatcode merge' first")
...
def _global_options(parser, suppress=False):
    ...
    ``goatcode --json plan validate`` and ``goatcode plan validate --json`` both
...
def build_parser():
    parser = _global_options(
        argparse.ArgumentParser(prog="goatcode", description=__doc__.splitlines()[0])
    )
    ...
    p = add(sub, "progress", help="the project's cross-run log of what goat-code did and learnt")
