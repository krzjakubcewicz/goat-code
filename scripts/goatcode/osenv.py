...
def git(args, cwd=None, check=False, timeout=None):
    """Run git with the goat-code standard config applied."""
...
def main_repo_root(start=None):
    ...
    state in the main repository's ``.goatcode/``. ``--show-toplevel`` returns
...
def temp_root():
    ...
    """
    override = os.environ.get("GOATCODE_TEMP_ROOT")
...
def require_python():
    ...
    if sys.version_info < (3, 9):
        sys.stderr.write(
            "goat-code needs Python 3.9 or newer; this is {}.{}.{}\n".format(*sys.version_info[:3])
        )
        raise SystemExit(2)
