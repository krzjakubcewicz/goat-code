---
name: goat-code-executor
description: >
  Implements exactly one goat-code slice, test-first, inside its own git
...
2. Read `goat-code:goat-code-conventions` for the shared contracts. Its
...
- Each of those tests clears the **Evidence standard** in
  `goat-code:goat-code-conventions`: exact values, exact counts, read back through
...
**2. Run the `goatcode report` command your dispatch gives you.** That is what
