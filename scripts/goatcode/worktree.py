...
def branch_name(run_id, slice_id):
    return "goatcode/{}/{}".format(run_id, slice_id)
...
def reap_orphans(repo):
    """Drop goat-code worktrees git still lists but whose directory is gone.
    ...
    for path, branch in existing(repo).items():
        if branch and branch.startswith("goatcode/") and not path.exists():
            osenv.git(["worktree", "remove", "--force", str(path)], cwd=repo)
