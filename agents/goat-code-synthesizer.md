---
name: goat-code-synthesizer
description: >
  Resolves merge conflicts and wiring breakage in the goat-code integration
  ...
  report. Use only when `goatcode merge` reports a conflict or the integration
  build breaks.
...
needs judgement. Read `goat-code:goat-code-conventions` for the contracts, then
...
4. Continue the merge:
   ```bash
   python "${CLAUDE_PLUGIN_ROOT}/scripts/goatcode.py" merge --continue
   ```
