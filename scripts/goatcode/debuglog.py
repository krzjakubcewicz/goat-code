"""Debug trace: everything goat-code does, one line at a time.
...
Off unless asked for. ``GOATCODE_DEBUG=1`` for a one-off, ``debug: true`` in
config when you want it on for a while.

Deliberately imports nothing from the rest of ``goatcode``: ``osenv`` calls
...
#: Values of GOATCODE_DEBUG that mean "on". Anything else, including the empty
#: string, means off - so `GOATCODE_DEBUG=` in a shell profile does not
#: silently enable it.
...
def enabled():
    ...
    from_env = os.environ.get("GOATCODE_DEBUG")
...
def configure(from_config=None):
    """Record the config setting. ``GOATCODE_DEBUG`` still overrides it."""
    global _ENABLED
